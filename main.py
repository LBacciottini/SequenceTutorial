...
class RouterNode(QuantumRouter):
    def __init__(self, name, tl, memo_size=50):
        super().__init__(name, tl, memo_size=memo_size)
...
from sequence.entanglement_management.generation import BarretKokA
...
def eg_req_func(protocols, args):
    ...
    for protocol in protocols:
        if not isinstance(protocol, BarretKokA):
            continue
...
def eg_rule_action1(memories_info: List["MemoryInfo"], args):
    ...
    memory = memories[0]
    protocol = BarretKokA(None, "EGA." + memory.name, mid_name,
                                       other_name,
                                       memory)
...
def eg_rule_action2(memories_info: List["MemoryInfo"], args):
    ...
    memories = [info.memory for info in memories_info]
    memory = memories[0]
    protocol = BarretKokA(None, "EGA." + memory.name,
...
def add_eg_rules(index: int, path: List[RouterNode], middles: List[BSMNode]):
    ...
    if index < (len(path) - 1):
        ...
        rule = Rule(10, eg_rule_action1, eg_rule_condition, action_args=action_args, condition_args=condition_args)
        node.resource_manager.load(rule)


from sequence.entanglement_management.purification import BBPSSWCircuit
